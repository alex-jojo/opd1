...
EXP="146_qwen3_4b_teacher_qwen3_4b_opd_rubric_reroll_hint_only_lr1e-6"
CKPT="/G-OPD-checkpoints/${EXP}_save_step_50"
RUN_DIR="/workspace/opd1/eval_runs/${EXP}"
SUMMARY_FILE="$RUN_DIR/summary_V12_qwen3_4b_reroll_hint_only_lr1e-6.csv"
...
GPT_ROLLOUT_SCORE_REROLL_HINT_LOSS_WEIGHT=0.50 \
GPT_ROLLOUT_SCORE_REROLL_NOHINT_ENABLE=False \
GPT_ROLLOUT_SCORE_REROLL_NOHINT_MIN_SCORE=50 \
GPT_ROLLOUT_SCORE_REROLL_NOHINT_MIN_GAIN=10 \
GPT_ROLLOUT_SCORE_REROLL_NOHINT_MAX_WEIGHT=0.5 \
...
if [ ! -s "$RUN_DIR/eval_outputs/summary.csv" ]; then
    echo "ERROR: eval summary is missing or empty: $RUN_DIR/eval_outputs/summary.csv"
    echo "Keeping checkpoint for manual recovery: $CKPT"
    exit 1
fi

cp "$RUN_DIR/eval_outputs/summary.csv" "$SUMMARY_FILE"
cat "$SUMMARY_FILE"

if [ ! -s "$SUMMARY_FILE" ]; then
    echo "ERROR: copied summary is missing or empty: $SUMMARY_FILE"
    echo "Keeping checkpoint for manual recovery: $CKPT"
    exit 1
fi
