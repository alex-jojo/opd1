"""Main entrypoint for the isolated EOPD baseline recipe."""
...
from recipe.eopd_baseline.eopd_ray_trainer import RayEOPDTrainer
...
class EOPDTaskRunner(PPOTaskRunner):
    """TaskRunner that swaps in the EOPD FSDP worker and trainer."""
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if config.actor_rollout_ref.actor.strategy not in {"fsdp", "fsdp2"}:
            raise NotImplementedError("The isolated EOPD baseline currently supports FSDP/FSDP2 only.")

        from recipe.eopd_baseline.eopd_fsdp_workers import ActorRolloutRefWorker, AsyncActorRolloutRefWorker
...
def main(config):
    run_ppo(config, task_runner_class=ray.remote(num_cpus=1)(EOPDTaskRunner))
