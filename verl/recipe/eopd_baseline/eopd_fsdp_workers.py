...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        from recipe.eopd_baseline.eopd_dp_actor import DataParallelPPOActor
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        eopd_config = policy_loss_config.get("eopd", {}) or {}
        eopd_enabled = bool(eopd_config.get("enable", policy_loss_config.get("eopd_enable", False)))
        if self._is_lora:
            if eopd_enabled:
                raise RuntimeError("EOPD baseline requires a standalone ref model to produce teacher top-k tensors.")
        ...
        topk = int(eopd_config.get("topk", ref_topk if ref_topk is not None else 16))
        renormalize_topk = bool(eopd_config.get("entropy_renormalize_topk", True))
        with self.ulysses_sharding_manager:
            ...
            if eopd_enabled:
                ...
                if entropy is None or topk_logps is None or topk_indices is None:
                    raise RuntimeError("EOPD ref policy did not return teacher entropy/top-k tensors.")
                output = DataProto.from_dict(
                    tensors={
                        "ref_log_prob": output,
