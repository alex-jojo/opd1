...
def _top_fraction_mask(entropys: torch.Tensor, response_mask: torch.Tensor, fraction: float) -> torch.Tensor:
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        entropy_opd_enabled = _env_flag("ENTROPY_OPD_ENABLE")
        if entropy_opd_enabled and "ref_entropys" in data.batch.keys():
        ...
        if "opd_rubric_adv_shift" in data.non_tensor_batch.keys():
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self.config.policy_loss.only_reverse_kl_advantages:
                        ...
                        if entropy_opd_enabled:
                            if "ref_entropys" not in model_inputs:
                                raise KeyError("ENTROPY_OPD_ENABLE=1 requires ref_entropys from compute_ref_log_prob.")
                            entropy_fraction = min(max(_env_float("ENTROPY_OPD_FRACTION", 0.2), 0.0), 1.0)
                            entropy_mask = _top_fraction_mask(
                                model_inputs["ref_entropys"],
                                response_mask,
                                entropy_fraction,
                            ).to(dtype=response_mask.dtype)
                            selected_tokens = entropy_mask.sum()
                            total_tokens = response_mask.sum().clamp_min(1)
                            advantages = advantages * entropy_mask
                            response_mask = response_mask * entropy_mask
                            micro_batch_metrics["actor/entropy_opd_selected_frac"] = (
                                selected_tokens / total_tokens
                            ).detach().item()
                        if "opd_rubric_adv_shift" in model_inputs:
                            with torch.no_grad():
                                rubric_adv_shift = torch.as_tensor(
                                    model_inputs["opd_rubric_adv_shift"],
                                    device=advantages.device,
                                    dtype=advantages.dtype,
                                )
                                while rubric_adv_shift.dim() < advantages.dim():
                                    rubric_adv_shift = rubric_adv_shift.unsqueeze(-1)
                            advantages = advantages + rubric_adv_shift
                        if "g_opd_loss_weight" in model_inputs:
                            with torch.no_grad():
                                loss_weight = torch.as_tensor(
                                    model_inputs["g_opd_loss_weight"],
                                    device=advantages.device,
                                    dtype=advantages.dtype,
                                )
                                loss_weight = torch.clamp(loss_weight, min=0.0)
                                while loss_weight.dim() < advantages.dim():
